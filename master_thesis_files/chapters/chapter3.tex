...
This problem can be solved by minimising the execution time of the ML model, or by placing the early detection system in a position that is several metres away from the expected elephant's path.
...
Cameras were built out of Raspberry Pi, a PIR sensor, an FLIR Lepton 2.5 camera, and batteries, all of which were enclosed in a plastic housing.
...
Designing a Neural Network with Edge Impulse is a much less involved process than the one we described above, as many of the steps of image preprocessing are automated.
...
For the processing block, we selected the image processing block as this was the only possible choice, and for the learning block, we selected Keras's Neural Network block.

Neural Network block is configurable.
