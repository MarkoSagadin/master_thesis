...
If we examine example on Figure \ref{conv1} we can see that input with a size 3 x 3, stride 1, padding 1 and filter with a size 2 x 2 produces output with size 4 x 4.
...
It provides support for Android and iOS devices, embedded Linux.
...
TFLite provides API in different languages, such as Java, Swift, Python and C++.
TFLite Micro uses C++ API, specifically C++11, which reuses large part of the general TensorFlow codebase.

TFLite Micro library does not require any specific microcontroller peripherals, it only expects that users implement their own \verb|printf| function.
This means that the same C++ code that implements ML inference can be compiled to run on a microcontroller or a personal computer.
...
Because microcontrollers come in a variety of different architectures, platforms and are produced by a large number of different vendors that offer their own versions of IDEs it is sometimes hard to port external libraries

Libary is on github, use it for embedded. 
Examples for different platforms, name them
Kernel implementations are shared.
API can use kernel implementations either for micros or pcs

How are you building them, no special tools needed, makefiles,
workflow from trained model on.
