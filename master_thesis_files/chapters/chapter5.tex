...
\subsection{ Current measurements and results}
...
\footnotetext{ nRF52's datasheet specifies current consumptions at many different conditions, which are: type of sleep mode (System ON or System OFF, latter loses execution context at wakeup), amount of RAM retention and type of wakeup event. Consumption can range from 0.95 \si{\micro\ampere} to 17.37 \si{\microampere}. Because Zephyr provides only abstract interface to power management, implementation of which is platform dependent, we can only guess which exact nRF52 sleep mode Zephyr uses. We assumed expected current consumption of 3.16 \si{\micro\ampere} as this is the specified current consumption in System ON mode with full RAM retention and LFRC set as a wakeup event.}
